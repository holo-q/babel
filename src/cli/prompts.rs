...
use babel::pager::{
    prepare_transcript_messages, transcript_palette, transcript_visible_lines, TranscriptRoleFilter,
};
...
pub async fn cmd_prompts(
    ...
    for session in sessions {
        ...
        let rows = prompt_rows_for_session(session, &messages, context_rows, token_budget);
        if !rows.is_empty() {
...
fn prompt_rows_for_session(
    ...
    for (idx, message) in messages.iter().enumerate().rev() {
        ...
        rows.push(PromptRow {
            ...
            context: prompt_context(messages, start, idx),
