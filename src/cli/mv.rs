...
use claude_babel::core::{BabelCore, MigrateOptions, ConflictingWindow};
use claude_babel::state::SessionState;
...
pub async fn cmd_mv(
    ...
    if !conflicts.is_empty() {
        ...
        let active_count = conflicts.iter()
            .filter(|c| matches!(c.state, SessionState::Thinking | SessionState::ToolUse))
            .count();
    ...
    if anxious {
        ...
        let migratable = conflicts.iter()
            .filter(|c| matches!(c.state, SessionState::Idle | SessionState::AwaitingInput))
            .count();
        if migratable > 0 {
            println!("Step 2: Migrate {} idle terminal(s)", migratable);
            for c in &conflicts {
                if matches!(c.state, SessionState::Idle | SessionState::AwaitingInput) {
    ...
    if !outcome.terminals_migrated.is_empty() {
        println!("{}✓ Migrated {} terminal(s): {:?}",
            prefix, outcome.terminals_migrated.len(), outcome.terminals_migrated);
    }

    if outcome.storage.project_folder_renamed {
        println!("{}✓ Renamed project folder ({} sessions)",
            prefix, outcome.storage.sessions_preserved);
    }

    if outcome.storage.history_entries_updated > 0 {
        println!("{}✓ Updated {} history entries",
            prefix, outcome.storage.history_entries_updated);
    }

    if !outcome.active_terminals.is_empty() {
        println!("⚠ {} active terminal(s) not migrated: {:?}",
            outcome.active_terminals.len(), outcome.active_terminals);
    }
